...
# Self-extraction into Anamnesis

You are the extractor. No separate model runs. *You* decide what was worth keeping
and write it through the existing memory write path. Anamnesis stores it as markdown
...
`python -m anamnesis.remember --project P --type T --title "..."` or the Python API
`anamnesis.api.remember_lessons([...], project="P")` (see docs/SELF_EXTRACTION.md).
