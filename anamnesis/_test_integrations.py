...
def test_lc_recall_to_documents_shape():
    ...
    md = docs[0]["metadata"]
    assert md["stem"] == "s1" and md["project"] == "pr" and md["source"] == "anamnesis"
...
def test_li_recall_to_nodes_shape():
    ...
    assert nodes[0]["metadata"]["stem"] == "s1" and nodes[0]["metadata"]["source"] == "anamnesis"
...
def test_lc_retriever_raises_without_langchain():
    ...
    try:
        lc.AnamnesisRetriever(project="p")
...
def test_lc_memory_load_save_and_flush_resets():
    # AnamnesisMemory is a plain class (no BaseMemory) — works without langchain.
    mem = lc.AnamnesisMemory(project="pr", memory_key="hist", k=3)
...
def test_li_retriever_raises_without_llamaindex():
    if li._HAVE_LI:
        return
    try:
        li.AnamnesisRetriever(project="p")
        assert False, "expected ImportError"
    except ImportError as e:
