...
Anamnesis — like almost every agent-memory system — is **static**: it injects memories but
...
The learned `θ` deploys through the **existing** `ANAMNESIS_RANKER=posterior` ranker (1A): its
log-linear weights `POST_W` are exactly the bandit's feature weights, env-settable
(`ANAMNESIS_POST_W_REL/FREQ/SAL`). So 1B needs **no** hollow new ranker mode — the research learns
