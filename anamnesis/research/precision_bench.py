...
purely in-process; never prints, saves, or commits any note text. Identical posture to real_trace.
    ...
    ANAMNESIS_VAULT=/path python research/precision_bench.py [--save]
    ANAMNESIS_VAULT=/path python research/precision_bench.py --rerank [--save]
    ANAMNESIS_VAULT=/path ANAMNESIS_RERANK_MODEL=qwen2.5:7b python research/precision_bench.py --rerank
...
RERANK_N = int(os.environ.get("ANAMNESIS_RERANK_N", "12"))     # cosine pool the LLM re-orders
RERANK_MODEL = os.environ.get("ANAMNESIS_RERANK_MODEL", m.OLLAMA_MODEL)
...
def main():
    ...
    print(bar)
    if len(notes) < 20:
        print(f"  only {len(notes)} embedded notes — set ANAMNESIS_VAULT to a real, populated store.")
        return
