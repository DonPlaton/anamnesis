...
*Companion to `research/precision_bench.py`. Reproduce: `ANAMNESIS_VAULT=/path python
...
"would changing `ANAMNESIS_EMBED_MODEL`, and nothing else, improve recall?" test. Each is embedded into
...
stays (`ANAMNESIS_EMBED_MODEL`, index self-invalidates on change) for English-@10-heavy stores to pick mxbai.
...
(`research/longmem_eval.py --xrerank`; ships as `anamnesis.reranker_ce`.)
...
- **Ships as opt-in (`anamnesis.reranker_ce`, `ANAMNESIS_XRERANK=1`, `pip install anamnesis-memory[reranker]`):**
