...
`anamnesis/research/longmem_eval.py` and `embedder_ab.py` expect:

```
anamnesis/research/data/longmemeval_oracle.json
```
...
```bash
python anamnesis/research/longmem_eval.py --embed     # embed pool+questions once (writes the caches below)
python anamnesis/research/longmem_eval.py             # report recall@k (fast)
python anamnesis/research/longmem_eval.py --xrerank   # + the trained cross-encoder ([reranker] extra, GPU)
python anamnesis/research/embedder_ab.py              # A/B local embedders (each pulled in Ollama)
```
