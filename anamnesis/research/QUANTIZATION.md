...
why Anamnesis needs no approximate-nearest-neighbour dependency to scale. Every number is
on the LongMemEval-oracle stand (940 sessions, 500 questions, local bge-m3), scored exactly
as `longmem_eval.py`. Reproduce with `python anamnesis/research/rnd_launch.py`.

## The setup

Anamnesis keeps three layers. The markdown notes are the source of truth. The JSON embedding
...
it ships as the opt-in `ANAMNESIS_EMBED_QUANT=binary`. The default stays float16 and the cache
...
`ANAMNESIS_EMBED_QUANT=binary` is a new opt-in for users with very large vaults who want a
...
published recall numbers for Anamnesis are unchanged, because the default ranker is untouched.

*Run it:* `python anamnesis/research/rnd_launch.py` (battery) and
`ANAMNESIS_EMBED_QUANT=binary python anamnesis/_test_quant.py` (regression).
