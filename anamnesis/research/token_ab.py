...
def _distill(text: str, cache: dict) -> str:
    """Distil a session into a compact Anamnesis-style note via local Ollama, cached by
    content hash (the Ollama calls are the slow part — re-runs are instant). This is the
    REAL mechanism: Anamnesis stores the distilled note, never the raw session."""
...
def vault_distillation_ab():
    """Independent angle: Anamnesis's real mechanism is DISTILLATION. Compare the
...
def distillation_ab(sample_n=40):
    """The REAL Anamnesis lever, measured: distil each retrieved session into a compact
...
def main():
    ...
    if lm:
        ...
        print("    Anamnesis adds a second lever the raw-session model omits: DISTILLATION")
