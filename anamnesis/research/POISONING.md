...
| iv | supersession-abuse (a lone note retires a corroborated true one) | corroboration-gated quarantine | **shipped (W7, opt-in `ANAMNESIS_QUARANTINE`)** |
...
- **Shipped (2026-06-17, W7+W8):**
  ...
  - the corroboration-gated **quarantine** is now wired into `write_typed_note`, **opt-in** via
    `ANAMNESIS_QUARANTINE=1`. A single-source note that is also suspicious (near-max confidence, or
