...
Anamnesis ranks with a hand-tuned stack: cosine relevance + an additive log-recurrence boost +
...
well-calibrated on held-out data, so `P(q│m)` genuinely predicts relevance, not just orders it.

## Shipped: `ANAMNESIS_RANKER=posterior`
