...
*Anamnesis research note — 2026-06-16. Reproducible: `python anamnesis/research/recurrence_ablation.py --save` (numpy; matplotlib for the figure). Seeded, CPU, ~seconds.*
...
Anamnesis shipped (avg recall@1 **0.81 vs 0.69**, normalized), matching the log-scaled
...
Anamnesis re-states a recurring lesson under a fresh date and carries a `recurrence`
...
- **Rankers.** Relevance = cosine. We min-max normalize relevance and recurrence
  ...
  `w ∈ [0,1]`. We compare the **linear** `(n−1)` recurrence form (what Anamnesis
...
therefore mis-specified across regimes. The **linear** `(n−1)` boost Anamnesis
...
`ANAMNESIS_ADAPTIVE_RECUR=0` restores the fixed
weight; `ANAMNESIS_AMBIGUITY_K` tunes the margin sensitivity.
...
Anamnesis also stores *when a belief was held* (`valid_from`/`valid_to`, supersession)
...
```bash
python anamnesis/research/recurrence_ablation.py --save     # recurrence: table + .{json,png}
python anamnesis/research/bitemporal_ablation.py  --save     # bi-temporal: table + .{json,png}
python anamnesis/research/longmem_eval.py --embed            # LongMemEval: embed pool (slow, once)
python anamnesis/research/longmem_eval.py --save             # LongMemEval: external recall@k
```

LongMemEval-oracle is downloaded separately into `research/data/` (gitignored):
`curl -sSL -o anamnesis/research/data/longmemeval_oracle.json
