...
# ceiling; the JUDGE is held at deepseek-chat (set ANAMNESIS_DEEPSEEK_MODEL=deepseek-chat) so the
...
def run():
    ...
    if XRERANK:
        ...
        if not rc.available():
            print("--xrerank needs torch+transformers (pip install anamnesis-memory[reranker])",
                  file=sys.stderr)
            sys.exit(1)
