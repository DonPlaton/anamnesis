...
`longmem_eval.py` uses for Anamnesis, so the columns are directly comparable. Anamnesis is
...
from its own file), so there is no metric drift between us and them.
    ...
    python research/head_to_head.py --only=anamnesis,mem0,langmem --save
...
EMBED_MODEL = os.environ.get("ANAMNESIS_EMBED_MODEL", "bge-m3")    # same embedder for all → fair
COMP_LLM = os.environ.get("H2H_LLM", "qwen2.5:3b")                 # competitor extraction LLM


def _args():
    ap = argparse.ArgumentParser()
    ap.add_argument("--only", default="", help="comma list: anamnesis,mem0,langmem,amem,cognee,zep")
...
# ── Anamnesis (re-scored through the same score(), apples-to-apples) ───────────

def run_anamnesis(data, pool) -> dict:
...
def run_mem0(data, pool) -> dict:
    ...
    bench_dir = Path(os.environ.get("H2H_DATA") or (Path(tempfile.gettempdir()) / "anamnesis_h2h"))
...
ADAPTERS = {"anamnesis": run_anamnesis, "mem0": run_mem0, "langmem": run_langmem,
...
def main():
    ...
    want = [s.strip() for s in ARGS.only.split(",") if s.strip()] or ["anamnesis", "mem0"]
    ...
    if "anamnesis" in ranked and len(ranked) > 1:
        a = ranked["anamnesis"]["recall@5"]
        for k, v in ranked.items():
            if k == "anamnesis":
                continue
            d = a - v["recall@5"]
            verb = "ahead of" if d > 0.01 else ("behind" if d < -0.01 else "tied with")
            print(f"  Anamnesis (hybrid) R@5 {a:.3f} — {verb} {k} ({v['recall@5']:.3f}, Δ{d:+.3f})")
    else:
        print("  Run ≥2 systems (e.g. --only=anamnesis,mem0) for a head-to-head verdict.")
    print("  NB: same embedder (bge-m3) for everyone → this isolates the MEMORY pipeline, not the")
    print("  embedder. Anamnesis uses calibrated score fusion (not rank-fusion); its opt-in trained")
