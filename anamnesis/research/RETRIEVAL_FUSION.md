...
A short, reproducible study of how Anamnesis fuses its semantic and lexical signals, and
...
`longmem_eval.py` does. Reproduce with `python anamnesis/research/longmem_eval.py`.
...
structure helps on a genuinely linked vault (where Anamnesis itself links a bug to its fix
...
chunk recovers the buried evidence turn. Anamnesis does not store raw sessions. It stores
...
Anamnesis needs no ANN dependency to reach a hundred thousand notes. Full study and the
no-database verdict: `research/QUANTIZATION.md`. It ships off by default as
`ANAMNESIS_EMBED_QUANT=binary`.
...
ranker, not a flattering variant. `ANAMNESIS_FUSION=rrf` keeps the old behaviour as a
...
*Run it:* `python anamnesis/research/longmem_eval.py [--xrerank]` and
`python anamnesis/research/head_to_head.py --only=anamnesis,mem0,langmem,amem`.
