#!/usr/bin/env python3
"""LlamaIndex integration for Anamnesis — a retriever over agent memory.
...
Recall goes through anamnesis.api (stdlib core, unchanged). The mapping helper
`recall_to_nodes` is framework-free and unit-tested without LlamaIndex installed; the
retriever class needs `pip install anamnesis-memory[llamaindex]`.
"""
import sys
...
INSTALL_HINT = "LlamaIndex not installed — `pip install anamnesis-memory[llamaindex]`"
...
if _HAVE_LI:
    class AnamnesisRetriever(BaseRetriever):
        """A LlamaIndex Retriever backed by Anamnesis recall — returns scored TextNodes."""
