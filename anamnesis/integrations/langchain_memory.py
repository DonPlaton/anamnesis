#!/usr/bin/env python3
"""LangChain integration for Anamnesis — a retriever and a chat-memory helper.

    from anamnesis.integrations.langchain_memory import AnamnesisRetriever
    retriever = AnamnesisRetriever(project="myproj", k=5)
    docs = retriever.invoke("how did we fix the OOM crash")        # → list[Document]

    from anamnesis.integrations.langchain_memory import AnamnesisMemory
...
`AnamnesisRetriever` is a real LangChain `BaseRetriever` (needs `pip install
anamnesis-memory[langchain]`). `AnamnesisMemory` implements the classic memory-variables
...
through `anamnesis.api`, so this adds nothing to the stdlib core.
"""
import sys
...
INSTALL_HINT = "LangChain not installed — `pip install anamnesis-memory[langchain]`"
    ...
    for r in results:
        out.append({
            ...
            "metadata": {"ntype": r.get("ntype"), "project": r.get("project"),
                         ...
                         "title": r.get("title"), "source": "anamnesis"},
        })
    return out
...
if _HAVE_LC:
    class AnamnesisRetriever(BaseRetriever):
        """A LangChain Retriever backed by Anamnesis recall. Drop into any chain or
...
class AnamnesisMemory:
    ...
    modern LCEL pipelines, prefer `AnamnesisRetriever`. Call `flush()`/`clear()` at each
