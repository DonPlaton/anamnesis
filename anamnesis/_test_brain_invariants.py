...
def set_profile(v):
    if v is None:
        ...
        os.environ["ANAMNESIS_PROFILE"] = v
...
os.environ["ANAMNESIS_SALIENCE_BOOST"] = "0"     # isolate from the F5 nudge for an exact comparison
with tempfile.TemporaryDirectory() as td:
    proj = seed_vault(td)
    ...
    try:
        ...
        for n, v in _orig.items():
            setattr(m, n, v)
os.environ.pop("ANAMNESIS_SALIENCE_BOOST", None)
...
try:
    ...
    for n, v in _p.items():
        setattr(m, n, v)
    os.environ.pop("ANAMNESIS_LOCAL_ONLY", None)
