#!/usr/bin/env python3
"""`anamnesis watch` — always-on auto-capture for ANY agent that logs to disk.
...
required, no cron to configure.
    ...
    python -m anamnesis.watch                 # auto-detect known agent log dirs, poll every 60s
    python -m anamnesis.watch --list          # show what WOULD be watched, then exit
    python -m anamnesis.watch --once          # one sweep then exit (great for cron / a smoke test)
    python -m anamnesis.watch --interval 30
    python -m anamnesis.watch --dir ~/logs --project myproj --agent mybot   # add an explicit target
    python -m anamnesis.watch --no-auto --dir ~/logs --agent mybot          # ONLY explicit targets
...
def _project_roots() -> list[Path]:
    ...
    roots = [Path.cwd()]
    raw = os.environ.get("ANAMNESIS_PROJECT_ROOTS", "")
...
def known_targets() -> list[Target]:
    ...
    # ANAMNESIS_LOCAL_ONLY to keep that agent's captures off any cloud backend.
...
def main() -> int:
    ...
    if args.list:
        if not targets:
            ...
            print("        Point it at one explicitly:  python -m anamnesis.watch --dir <logs> --agent <name>")
            return 0
    ...
    if not targets:
        print("[watch] nothing to watch — no known agent logs found and no --dir given.",
              file=sys.stderr)
        print("        Try:  python -m anamnesis.watch --list", file=sys.stderr)
        return 1
