...
research/W2_PRECISION.md. So this is the reranker Anamnesis ships, and it is OFF by default.

Enable with ANAMNESIS_XRERANK=1. Heavy deps (torch + transformers) are imported lazily
ONLY when enabled, so the stdlib core stays dependency-free for everyone else. Install:
    pip install anamnesis-memory[reranker]
The model (~2 GB) downloads from HuggingFace on first use and runs best on a GPU.
"""
import os

MODEL = os.environ.get("ANAMNESIS_XRERANK_MODEL", "BAAI/bge-reranker-v2-m3")
MAX_LEN = int(os.environ.get("ANAMNESIS_XRERANK_MAXLEN", "512"))
ENABLED = os.environ.get("ANAMNESIS_XRERANK", "0") != "0"
_state = {}
