...
def do_remember(a) -> int:
    # Single write path: delegate to the in-process library API (anamnesis.api),
    ...
    if not stem:
        ...
        # ANAMNESIS_QUARANTINE=1) a single uncorroborated note diverted to Quarantine/.
        ...
        print("[remember] not written to active memory: content is injection-shaped, or was "
              "quarantined as a single uncorroborated source (ANAMNESIS_QUARANTINE)",
              file=sys.stderr)
        return 2
