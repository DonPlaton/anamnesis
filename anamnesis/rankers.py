...
`retrieve_relevant` lazy-loads this module ONLY when `ANAMNESIS_RANKER=posterior` or
`ANAMNESIS_DIVERGENCE>0`, so the default hot path never imports it and the core file carries no
...
except ImportError:                       # invoked as a top-level module (anamnesis/ on sys.path)
    import memory_hook as m


def posterior_rerank(scores: dict, rec_of: dict) -> dict:
    """ANAMNESIS_RANKER=posterior (1A): rank by the explicit log-posterior
