...
sharpens the score when the local embedder is free.
    ...
    python -m anamnesis.anticipate "refactoring the orchestrator, touching prism_orchestrator.py" --project p
    python -m anamnesis.anticipate feedback <mistake-stem> false_alarm
...
BASE_TAU = float(os.environ.get("ANAMNESIS_ANTICIPATE_TAU", "0.22"))   # min risk to fire
...
FP_STEP = float(os.environ.get("ANAMNESIS_ANTICIPATE_FP_STEP", "0.06"))  # bar raise per false alarm
