...
|---|---|---|---|---|---|---|
| **Anamnesis** | **markdown + JSON under git**; no DB/server | **hybrid** semantic (bge-m3 local) + lexical **RRF**, recurrence-boost, lexical fallback | supersession (→`Superseded/`) + RESOLVES edges + bi-temporal **prototype** | **yes**: hooks · MCP · ingest | **fully local, $0**; cloud only for opt-in extraction; secret redaction | **trivial, files only** |
